...
object DataScaler {
  ...
  /**
    ...
    *
    * @param data       the data
    * @param scaleSizes map with entry - the scale's resolution and size
    * @return map with key = scale's size, and value = list of groups with data items
    *
    *         </br><b>Example</b>: for input data of size 100 there are: </br>
    ...
    *         and so on ...
    */
  def scaleByResolution(data: List[Double], scaleSizes: Map[Long, Double]): Map[Long, List[List[Double]]] = {
    val tmpResult = scaleSizes.map { case (scale, _) =>
      // TODO: too much Java like
      ...
      for (i <- 0 until scale.toInt) {
        ...
        if (rowData.size >= minimumSaturation) {
